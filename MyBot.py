...
hlt.send_init("the donald")


def find_nearest_enemy_direction(square):
	# arbitrary direction (NORTH)
    direction = NORTH
...
def attack_value(square):
    if square.owner == 0 and square.strength > 0:
        return square.production / square.strength
    else:
        # total potential damage when attacking this square
...
def get_move(square):
    target, direction = max(((neighbor, direction) for direction, neighbor in enumerate(game_map.neighbors(square))
                                ...
                                default = (None, None),
                                key = lambda t: attack_value(t[0]))
    if target is not None and target.strength < square.strength:
        return Move(square, direction)
	# arbitraty prodcution multiple (5)
